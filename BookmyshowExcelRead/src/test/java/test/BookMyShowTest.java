...
public class BookMyShowTest extends BConfigFileInitialize{
	...
	public void bookTickets() throws IOException {
		row = ExcelRead.readRow("D:\\Java personal projects\\BookmyshowExcelRead\\MovieDetails.xlsx");
		
		driver.get(config.getProperty("bookmyshow.com"));
		
		homePage.getCity().click();
		homePage.getSearchFieldButton().click();
		homePage.getSearchFideld().sendKeys(row.get(0));
	...
	public void movieDetails() {
		...
		selectSeats(moviePage.getSeats(), row.get(1));
		moviePage.getSelectSeatButton().click();
	}
	
	@Test(dependsOnMethods = "movieDetails")
	public void seatSelection() {
		selectSeat(seatPage.getRows(), row.get(1));
	...
	public void loginOTP() {
		...
		Home_page.Entermob().sendKeys("7895080077");
	...
	public void form_fill_test() throws InterruptedException, IOException {
		...
		// ------------------------------------//
		page.List_your_show_page.user_id_twitter().sendKeys("deadpol830");

		// -----------------------enter password in twitter login
		// --------------------------------//
		page.List_your_show_page.passwd_twitter().sendKeys("jumbo@123");
